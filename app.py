...
def analyze():
    ...
    for sc in KANA_LIST:
        ...
        def solve(path):
            ...
            # --- 牛耕の新ルール反映 ---
            # 奇数番目結合(1→2語目, 3→4語目...): 尻と尻を繋ぐ
            # 偶数番目結合(2→3語目, 4→5語目...): 頭と頭を繋ぐ
            is_odd_connection = (len(path) % 2 != 0)
            
            if round_trip:
                target_pos = "tail" if is_odd_connection else "head"
                src_char = get_clean_char(path[-1], target_pos) # 前の単語の同じ側を基準にする
            else:
                # 通常：常に前の尻と次の頭を繋ぐ
                target_pos = "head"
                src_char = get_clean_char(path[-1], "tail")
