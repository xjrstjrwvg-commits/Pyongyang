...
def analyze():
    ...
    for sc in KANA_LIST:
        ...
        def solve(path):
            if time.time() - start_time > 55: return # タイムアウトチェック
            ...
            target_chars = [shift_kana(src_char, ks_val), shift_kana(src_char, -ks_val)] if use_shift and ks_val != 0 else [src_char]
            
            target_pos = "tail" if is_rt_step else "head"
            
            # インデックスを使用して検索を高速化
            # (target_posがheadの場合はインデックスが使えるが、tailの場合は全探索が必要)
            # 牛耕（round_trip）がOFFなら常にhead_indexが使える
            if target_pos == "head":
                ...
                for nxt in word_pool:
                    if nxt not in path and get_clean_char(nxt, "tail") in target_chars:
                        solve(path + [nxt])
